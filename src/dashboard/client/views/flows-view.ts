/** <bk-flows-view> — User action flows with simple/detailed view modes. */
...
export class FlowsView extends LitElement {
  ...
  private loadTimelineForContainer(container: HTMLElement) {
    ...
    for (const tlEl of tlEls) {
      ...
      if (rid && !tlEl.hasAttribute("data-loaded")) {
        ...
        panel.setAttribute("request-started", tlEl.getAttribute("data-request-started") || "0");
  ...
  render() {
    ...
    if (flows.length === 0) {
      return html`<bk-empty-state title="No actions yet" subtitle="Start using your app to see user action flows here"></bk-empty-state>`;
    }
    ...
    return html`
      ...
      <div id="flow-list">${flows.map((flow, idx) => this.renderFlowRow(flow, idx))}</div>
    `;
  }
  ...
  private renderFlowRow(flow: FlowData, idx: number) {
    ...
    return html`
      <div class="flow-row ${isExpanded ? "expanded" : ""}" @click=${() => this.toggleFlow(idx)}>
        <div class="flow-summary-row">
          <span class="flow-status-dot ${dotClass}"></span>
          <span class="flow-label">${flow.label}</span>
          <span class="flow-req-count">${flow.requests.length} req${flow.requests.length !== 1 ? "s" : ""}</span>
          <span class="flow-badge-pill ${badge.cls}">${badge.text}</span>
          <span class="flow-duration">${formatDuration(flow.totalDurationMs)}</span>
        </div>
      </div>
  ...
  private renderFlowInsights(flow: FlowData) {
    ...
    return html`
      <div>
        <div class="flow-traffic">${flow.requests.map((req) => this.renderTrafficCard(req))}</div>
        ${hasIssues ? html`
          <div class="flow-divider"></div>
          <div class="flow-insights">
            ${insights.errors.map((err) => html`<div class="insight-line insight-error">\u2717 ${err}</div>`)}
            ${insights.duplicates.map((dup) => html`<div class="insight-line insight-warn">\u26A0 ${dup.name} \u2014 loaded ${dup.count}x (wasting ~${formatDuration(dup.wastedMs)})</div>`)}
            ${insights.warnings.map((w) => html`<div class="insight-line insight-warn">\u26A0 ${w}</div>`)}
            ${insights.tip ? html`<div class="insight-line insight-tip">Tip: ${insights.tip}</div>` : nothing}
          </div>
        ` : nothing}
      </div>
    `;
  ...
  private renderTrafficCard(req: FlowRequest) {
    ...
    const hasDetails =
      (!req.isDuplicate && req.category !== CATEGORY_STATIC && req.category !== CATEGORY_POLLING) ||
      (req.requestBody && req.method !== "GET") ||
      !!req.responseBody;

    return html`
      <div class="traffic-card ${req.isStrictModeDupe ? "strict-mode-dupe" : ""}">
        <div class="traffic-card-header ${hasDetails ? "has-details" : ""}">
          <bk-method-badge .method=${req.method}></bk-method-badge>
          <span class="traffic-card-path ${req.isDuplicate ? "is-dup" : ""}">${req.label}</span>
          ...
          ${req.isDuplicate
            ...
            : html`<span class="traffic-card-size">${formatSize(req.responseSize)}</span>`}
        </div>
        ${req.isStrictModeDupe
          ? html`<div class="strict-mode-banner">React Strict Mode duplicate \u2014 does not happen in production</div>`
          : nothing}
        ${!req.isDuplicate && req.category !== CATEGORY_STATIC && req.category !== CATEGORY_POLLING
          ? html`<div class="request-timeline tl-hidden" data-request-id=${req.id} data-request-started=${String(req.startedAt)}></div>`
          : nothing}
        ${req.requestBody && req.method !== "GET"
    ...
    return html`
      <div class="traffic-body">
        <button class="traffic-body-toggle" @click=${this.toggleBodyBlock}>
          <span class="chevron">\u25B8</span><span class="arrow-${direction}">${arrowChar}</span> ${label}
        </button>
  ...
  private renderFlowSubReqs(flow: FlowData) {
    return html`<div class="flow-subreqs">${flow.requests.map((req, idx) => this.renderSubReqRow(req, idx))}</div>`;
  }

  private renderSubReqRow(req: FlowRequest, idx: number) {
    const isExpanded = this.expandedSubReqIdx === idx;
    const sClass = statusPillClass(req.statusCode);
    const dur = req.pollingDurationMs ? formatDuration(req.pollingDurationMs) : formatDuration(req.durationMs);

    return html`
      <div class="flow-subreq ${isExpanded ? "expanded" : ""}" @click=${(e: Event) => this.toggleSubReq(idx, e)}>
        <bk-method-badge .method=${req.method}></bk-method-badge>
        <span class="subreq-label ${req.isDuplicate ? "is-dup" : ""}">${req.path || req.url}</span>
        ${req.isDuplicate ? html`<span class="subreq-dup-tag">duplicate</span>` : nothing}
        <span class="status-pill ${sClass}">${req.statusCode}</span>
  ...
  private renderSubReqDetail(req: FlowRequest) {
    ...
    return html`
      <div class="detail-meta">
        <span><bk-method-badge .method=${req.method}></bk-method-badge> ${escHtml(req.url)}</span>
        <span><span class="status-pill ${sClass}">${req.statusCode}</span></span>
      ...
      <div class="detail-actions">
        <button class="btn btn-curl" @click=${(e: Event) => { e.stopPropagation(); copyAsCurl(req); }}>Copy cURL</button>
      </div>
    `;
  }
}
