...
.tel-duration{width:70px;text-align:right;color:var(--dim)}
.tel-timestamp{width:130px;text-align:right;color:var(--dim)}
...
.tel-mono{font-family:monospace;font-size:12px}
.tel-clickable{cursor:pointer}
.error-stack{padding:8px 16px;font-size:11px;color:var(--text-dim);white-space:pre-wrap;font-family:var(--mono);background:var(--bg);border-bottom:1px solid var(--border)}

/* Query rows */
.query-row{display:flex;align-items:center;gap:16px;font-family:var(--mono);font-size:13px}
...
.query-preview{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dim);font-size:11px;border-right:1px solid var(--border-subtle);padding-right:16px}
